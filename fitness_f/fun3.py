...
def fenics_fitness(points,outside_tri):
    ...
    def lateral_sides(x, on_boundary):
        return (near(x[0], -0.125)) and on_boundary
    # def end_side(x, on_boundary):
    #     return (near(x[0], 0.250)) and on_boundary
    def bottom(x, on_boundary):
        return x[1]-x[0]<0.0376 and on_boundary
    ...
    bottomm = AutoSubDomain(lambda x: x[1]-x[0]<0.0376)
    # class bottomm(SubDomain):
    #     def inside(self, x, on_boundary):
    #         return x[1]-x[0]<0.0374
    g = Constant((-5e6, 5e7))
    ...
    von_Mises = sqrt(abs(3./2*inner(s/1e3, s/1e3)))
    ...
    # print(sum(np.isnan(von_Mises.vector()[:])))
    # print(max_vm_stress)
